...
function renderExerciseDetail() {
  ...
  const tm = Math.floor(State.timerTarget / 60);
  const ts = State.timerTarget % 60;
  const goalLabel = tm > 0 ? `${tm}m ${ts > 0 ? ts + 's' : ''}` : `${ts}s`;
  const tLeft = State.timerLeft;
  const tlm = Math.floor(tLeft / 60);
  const tls = tLeft % 60;
  const timerPct = Math.round((tLeft / State.timerTarget) * 100);
  ...
  return `<div class="screen">
    ...
    <div class="content" style="padding-bottom:140px;">
      ...
      <div class="timer-card">
        <div class="timer-label">Exercise Timer</div>
        <div class="timer-count${tLeft === 0 ? ' done' : ''}" id="timer-num">${tlm}:${String(tls).padStart(2,'0')}</div>
        <div class="timer-bar-bg">
          <div class="timer-bar-fill${tLeft === 0 ? ' done' : ''}" id="timer-bar" style="width:${timerPct}%"></div>
        </div>
        <div class="timer-adjust">
          <button class="timer-adj-btn" id="timer-minus">−10s</button>
          <span class="timer-target" id="timer-target-lbl">Goal: ${goalLabel}</span>
          <button class="timer-adj-btn" id="timer-plus">+10s</button>
        </div>
        <div class="timer-btns">
          <button class="timer-start-btn" id="timer-start">${State.timerRunning ? '⏸ Pause' : tLeft === 0 ? '↺ Reset' : '▶ Start'}</button>
          <button class="timer-reset-btn" id="timer-reset">↺</button>
        </div>
      </div>
    </div>
    ...
    <div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:var(--bg);border-top:1px solid var(--border);">
      ...
      <div style="padding:12px 16px calc(var(--safe-bot) + 10px);">
        ${skipped
          ? `<div class="warn-box" style="margin-bottom:8px;"><span>↷</span><span>Skipped today — reason: ${skips[ex.id].reason === 'too-difficult' ? 'Too difficult' : skips[ex.id].reason === 'dislike' ? 'Dislike' : 'Other'}${skips[ex.id].text ? ': ' + escHtml(skips[ex.id].text) : ''}</span></div>
...
function bindExerciseDetail() {
  ...
  document.getElementById('timer-minus').addEventListener('click', () => adjustTimer(-10));
  document.getElementById('timer-plus').addEventListener('click', () => adjustTimer(10));
}
...
function resetExTimer() {
  ...
  if (numEl) numEl.classList.remove('done');
  if (barEl) barEl.classList.remove('done');
  updateTimerDisplay();
}

function adjustTimer(delta) {
  if (State.timerRunning) return;
  State.timerTarget = Math.max(10, State.timerTarget + delta);
  State.timerLeft   = State.timerTarget;
  const sb = document.getElementById('timer-start');
  if (sb) sb.textContent = '▶ Start';
  updateTimerDisplay();
...
function updateTimerDisplay() {
  ...
  const numEl = document.getElementById('timer-num');
  const barEl = document.getElementById('timer-bar');
  const lbl   = document.getElementById('timer-target-lbl');
  if (numEl) numEl.textContent = `${m}:${String(s).padStart(2, '0')}`;
  if (barEl) barEl.style.width = `${(State.timerLeft / State.timerTarget) * 100}%`;
  if (lbl) {
    const tm = Math.floor(State.timerTarget / 60);
    const ts = State.timerTarget % 60;
    lbl.textContent = `Goal: ${tm > 0 ? tm + 'm ' : ''}${ts}s`;
  }
}
