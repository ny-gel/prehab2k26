...
const Store = {
  ...
  bestStreak() {
    const first = Store.firstActivityKey();
    if (!first) return 0;
    let best = 0, run = 0;
...
function renderHome(profile) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (d.getTime() === todayDate.getTime()) {
      ...
      glyph = allDone ? '✓' : '!';
    } else if (d > todayDate) { cls = 'wk-future'; glyph = '–'; }
...
function summaryWeeklyGrid() {
  ...
  const pct21 = Math.round((complete / 21) * 100);
...
function renderSummary(profile) {
  ...
  const exRows = EXERCISES.map(e => `
    <tr>
      <td>
        <div class="sum-ex-name">${e.name}</div>
        ${e.muscles ? `<div class="sum-ex-muscle">${e.muscles}</div>` : ''}
      </td>
      <td>${counts[e.id]}</td>
